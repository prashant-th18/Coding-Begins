...
int32_t main() {
    ...
    for(int i = 1001; i <= 9999; ++i) if(v[i]) all_primes.push_back(i);
    for(const auto& val : all_primes)
    {
        string res = to_string(val);
        for(int i = 0; i < 4; ++i)
        {
            // i determines which index must be changed
            string temp = res;
